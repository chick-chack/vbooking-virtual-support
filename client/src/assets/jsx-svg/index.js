import AddEventSVG from "./AddEventSVG";
import AddFriendSVG from "./AddFriendSVG";
import AddImageSVG from "./AddImageSVG";
import AddSVG from "./AddSVG";
import AddSVG1 from "./AddSVG1";
import AddUserSVG from "./AddUserSVG";
import AdminSVG from "./AdminSVG";
import AppleSVG from "./AppleSVG";
import ArrowDownSVG from "./ArrowDownSVG";
import ArrowRightSVG from "./ArrowRightSVG";
import BlackShareSVG from "./BlackShareSVG";
import BusinessBagSVG from "./BusinessBagSVG";
import CalenderBtnSVG from "./CalenderBtnSVG";
import CalenderSVG from "./CalenderSVG";
import CallEndSVG from "./CallEndSVG";
import CallSVG from "./CallSVG";
import CartSVG from "./CartSVG";
import ChatSVG from "./ChatSVG";
import CitySVG from "./CitySVG";
import CloseSVG from "./CloseSVG";
import CommentSVG from "./CommentSVG";
import DropdownSVG from "./DropdownSVG";
import DeleteSVG from "./DeleteSVG";
import DimensionsSVG from "./DimensionsSVG";
import EditIconSVG from "./EditIconSVG";
import EditSVG from "./EditSVG";
import ExitFullscreenSVG from "./ExitFullscreenSVG";
import ExpandSVG from "./ExpandSVG";
import FacebookSVG from "./FacebookSVG";
import FiltersSVG from "./FiltersSVG";
import FriendSVG from "./FriendSVG";
import FullscreenSVG from "./FullscreenSVG";
import GoogleSVG from "./GoogleSVG";
import GridSVG from "./GridSVG";
import GroupsSVG from "./GroupsSVG";
import HologramSVG from "./HologramSVG";
import HomeSVG from "./HomeSVG";
import LawSVG from "./LawSVG";
import LeftArrowSVG from "./LeftArrowSVG";
import LikeSVG from "./LikeSVG";
import ListIcon2SVG from "./ListIcon2SVG";
import ListIconSVG from "./ListIconSVG";
import LinkSVG from "./LinkSVG";
import LocationSVG from "./LocationSVG";
import LogoSVG from "./LogoSVG";
import ListSVG from "./ListSVG";
import LogoutSVG from "./LogoutSVG";
import ManGroupSVG from "./ManGroupSVG";
import MarketPlaceSVG from "./MarketPlaceSVG";
import MeetingCallSVG from "./MeetingCallSVG";
import MessageSVG from "./MessageSVG";
import MetaverseSVG from "./MetaverseSVG";
import MoreSVG from "./MoreSVG";
import MoreVerticalSVG from "./MoreVerticalSVG";
import MuteVoiceSVG from "./MuteVoiceSVG";
import NoSoundSVG from "./NoSoundSVG";
import NotificationSVG from "./NotificationSVG";
import NoVideoSVG from "./NoVideoSVG";
import OptionsSVG from "./OptionsSVG";
import PackageSVG from "./PackageSVG";
import PaperclipSVG from "./PaperclipSVG";
import ParagraphSVG from "./ParagraphSVG";
import ParticipantsSVG from "./ParticipantsSVG";
import PhotoSVG from "./PhotoSVG";
import PihiWorldSVG from "./PihiWorldSVG";
import PiSVG from "./PiSVG";
import PlusSVG from "./PlusSVG";
import PostSVG from "./PostSVG";
import ProfileSVG from "./ProfileSVG";
import RecAddSVG from "./RecAddSVG";
import RenameSVG from "./RenameSVG";
import ScreenSVG from "./ScreenSVG";
import SearchSVG from "./SearchSVG";
import SendSVG from "./SendSVG";
import SettingFillSVG from "./SettingFillSVG";
import SettingsGearSVG from "./SettingsGearSVG";
import SettingsSVG from "./SettingsSVG";
import ShareArObjectSVG from "./ShareArObjectSVG";
import ShareDimenstionSVG from "./ShareDimenstionSVG";
import ShareScreenSVG from "./ShareScreenSVG";
import ShareSVG from "./ShareSVG";
import ShoppingCartSVG from "./ShoppingCartSVG";
import SliderSVG from "./SliderSVG";
import SortSVG from "./SortSVG";
import SoundSVG from "./SoundSVG";
import StepAppsSVG from "./StepAppsSVG";
import StepCommentSVG from "./StepCommentSVG";
import StepFormSVG from "./StepFormSVG";
import StepRouteSVG from "./StepRouteSVG";
import TimeSVG from "./TimeSVG";
import ToggleFullScreenSVG from "./ToggleFullScreenSVG";
import TrendUpSVG from "./TrendUpSVG";
import UsDollarSVG from "./UsDollarSVG";
import VerifiedSVG from "./VerifiedSVG";
import VideoSVG from "./VideoSVG";
import VirtualMeetSVG from "./VirtualMeetSVG";
import VoiceSVG from "./VoiceSVG";
import VverseCrownSVG from "./VverseCrownSVG";
import VverseElearningSVG from "./VverseElearningSVG";
import VverseGrowSVG from "./VverseGrowSVG";
import VverseMegaphoneSVG from "./VverseMegaphoneSVG";
import VversePercentageSVG from "./VversePercentageSVG";
import VversePuzzleSVG from "./VversePuzzleSVG";
import SmileySVG from "./SmileySVG";
import SwimSVG from "./SwimSVG";
import LiveSVG from "./LiveSVG";
import HatSVG from "./HatSVG";
import EventSVG from "./EventSVG";
import EntertainmentSVG from "./EntertainmentSVG";
import AlbumsSVG from "./AlbumsSVG";
import DimSVG from "./DimSVG";
import AirplaneSVG from "./AirplaneSVG";
import WalletSVG from "./WalletSVG";
import FavoriteSVG from "./FavoriteSVG";
import SwitchAccountsSVG from "./SwitchAccountsSVG";
import ArchiveSVG from "./ArchiveSVG";
import GradiantArrowSVG from "./GradiantArrowSVG";
import WebViewSVG from "./WebViewSVG";
import SMSSVG from "./SMSSVG";
import ToolsSVG from "./ToolsSVG";
import WorldMapSVG from "./WorldMapSVG";
import WhiteBoardSVG from "./WhiteBoardSVG";
import DestinationSVG from "./DestinationSVG";
import BookingsExperienceSVG from "./BookingsExperienceSVG";
import HotelsSVG from "./HotelsSVG";
import FlightsSVG from "./FlightsSVG";
import CalculatorSVG from "./CalculatorSVG";
import ExploreLocationSVG from "./ExploreLocationSVG";
import PersonSVG from "./PersonSVG";

export {
  AddEventSVG,
  AddFriendSVG,
  AddImageSVG,
  AddSVG,
  AddSVG1,
  AddUserSVG,
  AdminSVG,
  AppleSVG,
  ArrowDownSVG,
  ArrowRightSVG,
  BlackShareSVG,
  BusinessBagSVG,
  CalenderBtnSVG,
  CalenderSVG,
  CallEndSVG,
  CallSVG,
  CartSVG,
  ChatSVG,
  CitySVG,
  CloseSVG,
  CommentSVG,
  DropdownSVG,
  DeleteSVG,
  EditIconSVG,
  EditSVG,
  ExitFullscreenSVG,
  ExpandSVG,
  FacebookSVG,
  FiltersSVG,
  FriendSVG,
  FullscreenSVG,
  GoogleSVG,
  GridSVG,
  GroupsSVG,
  HologramSVG,
  HomeSVG,
  LawSVG,
  LeftArrowSVG,
  LikeSVG,
  ListIcon2SVG,
  ListIconSVG,
  LinkSVG,
  ListSVG,
  LocationSVG,
  LogoSVG,
  LogoutSVG,
  ManGroupSVG,
  MarketPlaceSVG,
  MeetingCallSVG,
  MessageSVG,
  MetaverseSVG,
  MoreSVG,
  MoreVerticalSVG,
  MuteVoiceSVG,
  NoSoundSVG,
  NotificationSVG,
  NoVideoSVG,
  OptionsSVG,
  PackageSVG,
  PaperclipSVG,
  ParagraphSVG,
  ParticipantsSVG,
  PihiWorldSVG,
  PiSVG,
  PlusSVG,
  ProfileSVG,
  RecAddSVG,
  RenameSVG,
  ScreenSVG,
  SearchSVG,
  SendSVG,
  SettingFillSVG,
  SettingsGearSVG,
  SettingsSVG,
  ShareArObjectSVG,
  ShareDimenstionSVG,
  ShareScreenSVG,
  ShareSVG,
  ShoppingCartSVG,
  SliderSVG,
  SortSVG,
  SoundSVG,
  StepAppsSVG,
  StepCommentSVG,
  StepFormSVG,
  StepRouteSVG,
  TimeSVG,
  ToggleFullScreenSVG,
  TrendUpSVG,
  UsDollarSVG,
  VerifiedSVG,
  VideoSVG,
  VirtualMeetSVG,
  VoiceSVG,
  VverseCrownSVG,
  VverseElearningSVG,
  VverseGrowSVG,
  VverseMegaphoneSVG,
  VversePercentageSVG,
  VversePuzzleSVG,
  SmileySVG,
  SwimSVG,
  LiveSVG,
  HatSVG,
  EventSVG,
  EntertainmentSVG,
  AlbumsSVG,
  DimSVG,
  AirplaneSVG,
  WalletSVG,
  FavoriteSVG,
  SwitchAccountsSVG,
  ArchiveSVG,
  GradiantArrowSVG,
  PhotoSVG,
  PostSVG,
  DimensionsSVG,
  WebViewSVG,
  SMSSVG,
  ToolsSVG,
  WhiteBoardSVG,
  WorldMapSVG,
  BookingsExperienceSVG,
  DestinationSVG,
  HotelsSVG,
  FlightsSVG,
  PersonSVG,
  ExploreLocationSVG,
  CalculatorSVG,
};
